...
Patchwork protocol lets users or developers mint new Patch NFTs, or Patch NFTs soulbound to existing NFTs. These Patches enforce read/write/whitelist/transfer rules for Fragment NFTs, which themselves are NFT metadata primitives (eg., XP, coupons, in-game items, pfp attributes like a red hat). 

![Overview](PatchworkOverview.png)
...
Developers express their application’s metadata and rules in json or via a UI wizard, and optimized contract code is automatically generated and deployed for them. The Patchwork protocol contract enforces data integrity, rules and relationships between assigned NFTs
